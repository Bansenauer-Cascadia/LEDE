...
public class ledeDB
{
    ...
    private static string DatabaseName = "LEDE_portal";
    ...
    public static void updateCoreRating(string KnowledgeC, string KnowledgeP, string KnowledgeR, int original_versid, int original_coretopicid, string CoreTopicName)
    {
        int foobar = 0;
        string sel = @"addcoretopicrating @coretopicid, @VersID, @UserID, '@ReviewDate',
                        @CScore, @PScore, @SScore";
        SqlConnection con = new SqlConnection(getConnectionString());
        SqlCommand cmd = new SqlCommand(sel, con);
        cmd.Parameters.AddWithValue("coretopicid", original_coretopicid);
        cmd.Parameters.AddWithValue("VersID", original_versid);
        cmd.Parameters.AddWithValue("ReviewDate", DateTime.Now);
        cmd.Parameters.AddWithValue("CScore", Convert.ToInt32(KnowledgeC));
        cmd.Parameters.AddWithValue("PScore", Convert.ToInt32(KnowledgeP));
        cmd.Parameters.AddWithValue("SScore", Convert.ToInt32(KnowledgeR));
    ...
    {
        string sel = "select id, (lastname + ', ' + firstname) as name from users";
        SqlConnection con = new SqlConnection(getConnectionString());
        SqlCommand cmd = new SqlCommand(sel, con);
        con.Open();
        SqlDataReader dr = cmd.ExecuteReader();
...
where taskversion.userid = @userid and taskversion.version = @versid and taskversion.taskid = @taskid)";
    ...
    {
        string sel =
@"select taskversion.versid, task.taskname as assignment, taskversion.version, 
document.uploaddate as 'Date Modified', taskversion.ratingstatus from taskversion
join task on taskversion.taskid = task.taskid
join users on taskversion.userid = users.id 
join document on document.documentid = taskversion.documentid
where users.id=@userid
order by ratingstatus";
        SqlConnection con = new SqlConnection(getConnectionString());
        SqlCommand cmd = new SqlCommand(sel, con);
        cmd.Parameters.AddWithValue("userid", userid);
        con.Open();
